...
const ANSWER_MODES: { mode: FlashcardMode; label: string; desc: string }[] = [
  {
    ...
    desc: "Pick the meaning from 4 options",
  },
  {
    mode: "recall",
    label: "Recall",
    desc: "Try to remember it yourself, then reveal",
  },
];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const entries = await Promise.all(
        DECKS.map(
          async (d) =>
            [d.kind, await countDue({ kind: d.kind, starredIds })] as const,
        ),
      );
      ...
      const entries = await Promise.all(
        lists.map(
          async (l) =>
            [
              l.id,
              await countDue({ kind: "list", listIds: l.itemIds }),
            ] as const,
        ),
      );
