...
function segmentsFor(surface, readingKata) {
  ...
  if (!reading || !wanakana.isKana(reading)) return [{ t: surface, f: undefined }];
...
function romajiFor(tok) {
  ...
  const pron = tok.pronunciation && tok.pronunciation !== "*" ? tok.pronunciation : tok.reading;
...
function startsNewChunk(tok, prev) {
  ...
  if (tok.pos_detail_1 === "接尾" || tok.pos_detail_1 === "非自立") return false;
  return true;
}
...
function analyze(tokenizer, jp) {
  ...
  morphemes.forEach((tok, i) => {
    ...
    if (ro) {
      ...
      else romaji += (romajiSpaceBefore(tok, prev) || romaji === "" ? (romaji === "" ? "" : " ") : "") + ro;
    }
  });
  ...
  const merged = chunks.map((chunk) => {
    ...
    for (const seg of chunk) {
      if (!seg.f && out.length && !out[out.length - 1].f) out[out.length - 1].t += seg.t;
      else out.push({ ...seg });
    }
    return out;
  });
...
kuromoji.builder({ dicPath: path.join(__dirname, "..", "node_modules", "kuromoji", "dict") }).build((err, tokenizer) => {
  ...
  for (const v of vocab) {
    const surface = (v.word || "").replace(/[（(].*?[）)]/g, "").replace(/[～\[\]［］]/g, "").trim();
    if (!surface) continue;
  ...
  console.log(`tokens added: ${gCount} grammar examples, ${dCount} dialogue lines; ${vCount} vocab romaji spaced.`);
});
